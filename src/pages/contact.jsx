...
const stepList = ["Start a Conversation about a new business", "Tell us more about you. What's your business information", "We'll get back to you soon. Until then."];
    ...
    <div className="flex dark:text-white text-black flex-col md:flex-row items-end justify-start">
      ...
      <div className="flex md:w-1/2 w-full relative z-10  flex-col items-start justify-between">
        <div className="flex flex-col w-full ">
          {step === 1 &&
            stepaList.map((step, ind) => (
              <button key={ind} onClick={() => setStepa(step)} className="footer-link flex items-center justify-start">
                <div className={`w-4 h-4 mr-2 border dark:border-white border-black rounded-full ${stepa === step ? "dark:bg-white bg-black" : ""}`}></div>
                {step}
              </button>
            ))}
          {step === 2 && (
        ...
        <div className="flex w-full items-center justify-between">
          {step > 1 && (
            <button onClick={() => prevStep()} className="btn disabled:bg-white/5 mt-12 px-12">
              Previous
            </button>
          )}
          <div className=""></div>
          {step < 3 && (
            <button onClick={() => nextStep()} className="btn disabled:bg-white/5 mt-12 px-12">
              Next
            </button>
          )}
          {step === 3 && (
